...
def preprocess_daily_data(file_path, csv_file_path):
    # Load the data
    df = pd.read_csv(file_path)
    
    # Convert 'timestamp' to datetime and remove the hour component
    df['timestamp'] = pd.to_datetime(df['timestamp']).dt.date
    
    # Group by the date and aggregate the total calls
    df_daily = df.groupby('timestamp').agg({'total_calls': 'first'}).reset_index()

    # Save the daily aggregated data to CSV
    df_daily.to_csv(csv_file_path, index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--combine_data", action="store_true", help="Combine data from two files and save to CSV")
    parser.add_argument("--preprocess_daily_data", action="store_true", help="Preprocess daily data and save to CSV")
    args = parser.parse_args()

    if args.combine_data:
        CombineAndSaveData(
            file_path1="/home/leon/project_haarlemmermeer/2024 dialogue volume per dag en uur.xlsx",
            file_path2="/home/leon/project_haarlemmermeer/2023 dialogue volume per dag en uur.xlsx",
            csv_file_path="/home/leon/project_haarlemmermeer/test.csv"
        )
    
    if args.preprocess_daily_data:
        preprocess_daily_data(
            file_path="/home/leon/project_haarlemmermeer/call_center_data.csv",
            csv_file_path="/home/leon/project_haarlemmermeer/callcenter_daily.csv"
        )